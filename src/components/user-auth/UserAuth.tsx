		...
		return (
			<div class="group relative">
				<button class="flex items-center gap-2">
					<span class="text-sm font-bold hidden md:block">{user.name}</span>
				...
				<div class="rounded-base border-border bg-background shadow-shadow absolute top-full right-0 mt-2 hidden w-48 border-2 group-focus-within:block group-hover:block">
					<div class="p-2">
						<Link href={`/u/${user.id}`} class="text-foreground text-sm underline decoration-main decoration-2">{user.email}</Link>
					</div>
					<div class="p-2">
						<Form action={signout}>
